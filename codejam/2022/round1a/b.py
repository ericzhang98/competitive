...
def complicated_solution():
    ...
    assert 1 <= diff < 10**9
    left, right = make_left_right(diff)
    larger.extend(list(right))
    smaller.extend(list(left))
    assert sum(larger) == sum(smaller)
    assert set(larger) | set(smaller) == set(A) | set(B)
    print(" ".join(map(str, larger)))
    return

def easy_solution():
    N = int(input())
    A = []
    # all powers of 2 from 2^0..2^29
    for i in range(30):
        A.append(2**i)
    # for i in range(69):
    #     A.append(1100 + i)
